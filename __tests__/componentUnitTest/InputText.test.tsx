...
import CustomTextInput from '../../component/CustomTextInput'; // Adjust the import path as necessary
...
describe('<CustomTextInput />', () => {
  ...
  it('updates the value when text is entered', () => {
    ...
    const { getByPlaceholderText } = render(
      <CustomTextInput value="" onChange={mockOnChange} placeholder="Enter text" />
    );
  ...
  it('displays the correct value in the input field', () => {
    const { getByPlaceholderText } = render(
      <CustomTextInput value="Test" onChange={() => {}} placeholder="Enter text" />
    );
